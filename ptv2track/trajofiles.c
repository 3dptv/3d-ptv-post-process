...
**
** input : trajofiles <prefixfilname> <firstframe> <lastframe>
**
...
int main(int argc, char **argv)
{
  ...
  int next[nmax], last[nmax], folderlast[nmax], foldernext[nmax], quali[nmax];
  double xcor[nmax], ycor[nmax], zcor[nmax], averagelength=0;

  FILE	*FILEOUT, *FILEIN;
  char	filein[100], fileout[100], prefix[50], command[50];

  if(argc != 4)
    {
      printf("\n Usage: trajo <fileprefix> <firstdatasetnum.> <lastdatasetnum.> !!!\n");
      exit(1);
    }
  ...
  for (i_seq=firstframe; i_seq <= lastframe; i_seq++)
      ...
      for (j = 0; j < vectors; j++)
	{
	  fscanf(FILEIN, "%d %d %lf %lf %lf %d\n",
		 &last[j], &next[j], &xcor[j], &ycor[j], &zcor[j], &quali[j] );
	}
      ...
      for (j = 0; j < vectors; j++)
	{
	  if (next[j] >= 0)
		...
		folderlast[j] = file;
		fprintf(FILEOUT, "%4i %7.3f %7.3f %7.3f %d\n",
			i_seq, xcor[j] ,ycor[j] ,zcor[j], quali[j]);
		fclose(FILEOUT);
		count[file]++;
		file++;
	      }
...
printf("%5d %6i\n", j, count[j]); */
	...
	fprintf(FILEOUT, "-1\n");
      ...
      averagelength+=count[j];
    }

  averagelength/=file;
  printf("trajectories: %d, averagelength: %.1f, totallength: %.1f\n",file, averagelength,file*averagelength);
/*
